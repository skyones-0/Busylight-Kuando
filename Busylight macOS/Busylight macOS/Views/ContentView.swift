        ...
        VStack(spacing: 20) {
            ...
            HStack(spacing: 12) {
                ElegantStepper(icon: "briefcase.fill", value: $manager.workTimeMinutes, range: 1...60)
                    .onChange(of: manager.workTimeMinutes) { 
                        manager.updateConfiguration()
                        UserInteractionLogger.shared.settingChanged(setting: "Pomodoro Work Time", value: "\(manager.workTimeMinutes) min")
                    }
                
                ElegantStepper(icon: "cup.and.saucer.fill", value: $manager.shortBreakMinutes, range: 1...30)
                    .onChange(of: manager.shortBreakMinutes) { 
                        manager.updateConfiguration()
                        UserInteractionLogger.shared.settingChanged(setting: "Pomodoro Short Break", value: "\(manager.shortBreakMinutes) min")
                    }
                
                ElegantStepper(icon: "sun.max.fill", value: $manager.longBreakMinutes, range: 1...60)
                    .onChange(of: manager.longBreakMinutes) { 
                        manager.updateConfiguration()
                        UserInteractionLogger.shared.settingChanged(setting: "Pomodoro Long Break", value: "\(manager.longBreakMinutes) min")
                    }
                
                ElegantStepper(icon: "arrow.clockwise", value: $manager.configuredSets, range: 1...10)
                    .onChange(of: manager.configuredSets) { 
                        manager.updateConfiguration()
                        UserInteractionLogger.shared.settingChanged(setting: "Pomodoro Sets", value: "\(manager.configuredSets)")
                    }
            }
            .padding(.horizontal, 24)
...
// MARK: - Focusable Button Style (Global)
                ...
                GlassCard(title: "Appearance", icon: "paintbrush.fill") {
                    VStack(spacing: 16) {
                        ...
                        VStack(spacing: 8) {
                            ...
                            .onChange(of: appDelegate.showInDock) { oldValue, newValue in
                            ...
                            .onChange(of: appDelegate.showInMenuBar) { oldValue, newValue in
                            ...
                            VStack(alignment: .leading, spacing: 2) {
                                HStack {
                                    ...
                                    Toggle("", isOn: $smartFeatures.calendarSyncEnabled)
                                        .toggleStyle(.switch)
                                        .controlSize(.small)
                                        .onChange(of: smartFeatures.calendarSyncEnabled) { oldValue, newValue in
                                            UserInteractionLogger.shared.calendarSyncToggled(enabled: newValue)
                                        }
                                }
                            ...
                            VStack(alignment: .leading, spacing: 4) {
                                HStack {
                                    ...
                                    Toggle("", isOn: $smartFeatures.workHoursEnabled)
                                        .toggleStyle(.switch)
                                        .controlSize(.small)
                                        .onChange(of: smartFeatures.workHoursEnabled) { oldValue, newValue in
                                            UserInteractionLogger.shared.workHoursToggled(enabled: newValue)
                                        }
                                }
                        ...
                        if smartFeatures.isDeepWorkActive {
                            ...
                            HStack(spacing: 8) {
                                DeepWorkButton(minutes: 60) {
                                    UserInteractionLogger.shared.deepWorkStarted(durationMinutes: 60)
                                    smartFeatures.startDeepWorkMode(durationMinutes: 60)
                                }
                                DeepWorkButton(minutes: 90) {
                                    UserInteractionLogger.shared.deepWorkStarted(durationMinutes: 90)
                                    smartFeatures.startDeepWorkMode(durationMinutes: 90)
                                }
                                DeepWorkButton(minutes: 120) {
                                    UserInteractionLogger.shared.deepWorkStarted(durationMinutes: 120)
    ...
    var body: some View {
            ...
            VStack(spacing: 16) {
                ...
                if mlManager.configuration?.isMLEnabled == true {
                    ...
                    HStack(spacing: 10) {
                        // Botón Entrenar
                        Button(action: {
                            UserInteractionLogger.shared.mlTrainingStarted()
                            Task {
                                trainingError = nil
                                do {
                                    try await mlManager.trainModel()
                                    UserInteractionLogger.shared.mlTrainingCompleted(accuracy: mlManager.modelAccuracy)
                                } catch MLError.insufficientData {
                                    trainingError = "Need 3+ days of work data (have \(mlManager.trainingDaysCollected))"
                                    BusylightLogger.shared.warning("ML Training failed: Insufficient data")
                                } catch {
                                    trainingError = error.localizedDescription
                                    BusylightLogger.shared.error("ML Training failed: \(error.localizedDescription)")
                                }
                            }
                        }) {
                            HStack(spacing: 4) {
                        ...
                        if !mlManager.canTrainModel() && !mlManager.isTraining {
                            Button(action: {
                                UserInteractionLogger.shared.mlDemoDataGenerated()
        ...
        .focusable(false)
                    ...
                    HStack {
                        ...
                        Toggle("", isOn: Binding(
                            ...
                            set: { 
                                mlManager.updateConfiguration(autoAdjust: $0)
                                UserInteractionLogger.shared.mlAutoAdjustToggled(enabled: $0)
                            }
                        ))
                        .toggleStyle(.switch)
                        .controlSize(.mini)
        ...
        .alert("Clear all training data?", isPresented: $showingClearConfirmation) {
            ...
            Button("Clear", role: .destructive) {
                UserInteractionLogger.shared.mlDataCleared()
...
struct HolidayCalendarView: View {
    ...
    private func addPredefinedCalendar(country: (code: String, name: String, flag: String)) {
        ...
        guard !calendars.contains(where: { $0.countryCode == country.code && $0.isSystemCalendar }) else {
            BusylightLogger.shared.warning("Calendario para \(country.name) ya existe")
            return
        }
        ...
        do {
            ...
            UserInteractionLogger.shared.holidayCalendarCreated(name: calendar.name, countryCode: country.code, customDatesCount: 0)
            BusylightLogger.shared.info("✅ Calendario predefinido agregado: \(calendar.name)")
        } catch {
            BusylightLogger.shared.error("❌ Error guardando calendario predefinido: \(error.localizedDescription)")
        }
    }
    
    private func addCustomCalendar() {
        guard !customCalendarName.isEmpty else {
            BusylightLogger.shared.warning("Nombre de calendario vacío")
            return
        }
        guard !selectedDates.isEmpty else {
            BusylightLogger.shared.warning("No hay fechas seleccionadas para el calendario")
            return
        }
        ...
        do {
            try modelContext.save()
            UserInteractionLogger.shared.holidayCalendarCreated(name: calendar.name, countryCode: "CUSTOM", customDatesCount: selectedDates.count)
            BusylightLogger.shared.info("✅ Calendario personalizado creado: \(calendar.name) con \(selectedDates.count) fechas")
            
            // Reset form
            customCalendarName = ""
            selectedDates = []
        } catch {
            BusylightLogger.shared.error("❌ Error guardando calendario personalizado: \(error.localizedDescription)")
        }
    }
    
    private func toggleCalendar(_ calendar: HolidayCalendar) {
        let newState = !calendar.isEnabled
        calendar.isEnabled = newState
        calendar.modifiedAt = Date()
        
        do {
            try modelContext.save()
            UserInteractionLogger.shared.holidayCalendarToggled(name: calendar.name, enabled: newState)
            BusylightLogger.shared.info("📅 Calendario '\(calendar.name)' \(newState ? "activado" : "desactivado")")
        } catch {
            BusylightLogger.shared.error("❌ Error cambiando estado del calendario: \(error.localizedDescription)")
        }
    }
    ...
    private func deleteCalendar(_ calendar: HolidayCalendar) {
        ...
        do {
            ...
            BusylightLogger.shared.info("🗑️ Calendario eliminado: \(name)")
        } catch {
            BusylightLogger.shared.error("❌ Error eliminando calendario: \(error.localizedDescription)")
        }
    }
}
